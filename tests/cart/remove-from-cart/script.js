...
async function main() {
  ...
  try {
    ...
    console.log('STEP_START:4:Check if item is added to cart')
    ...
    console.log('STEP_START:5:Remove item from cart')
    await page.goto('https://www.amazon.in/gp/cart/view.html')
    await page.waitForLoadState('load')
    const deleteButton = page.locator('input[value="Delete"]').first()
